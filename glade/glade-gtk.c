...
static void container_build_children(GladeXML *xml, GtkWidget *w,
...
static void menushell_build_children(GladeXML *xml, GtkWidget *w, GNode *node,
			   const char *longname) {
  GNode *childnode;
...
static void menuitem_build_children(GladeXML *xml, GtkWidget *w, GNode *node,
			       const char *longname) {
  GtkWidget *menu;
...
static void box_build_children(GladeXML *xml, GtkWidget *w, GNode *node,
			       const char *longname) {
  GNode *childnode;
  for (childnode = node->children; childnode; childnode = childnode->next) {
    GtkWidget *child = glade_xml_build_widget(xml, childnode, longname);
...
static void table_build_children(GladeXML *xml, GtkWidget *w, GNode *node,
			       const char *longname) {
  GNode *childnode;
  for (childnode = node->children; childnode; childnode = childnode->next) {
    GtkWidget *child = glade_xml_build_widget(xml, childnode, longname);
...
static void fixed_build_children(GladeXML *xml, GtkWidget *w, GNode *node,
			       const char *longname) {
  GNode *childnode;
  for (childnode = node->children; childnode; childnode = childnode->next) {
    GtkWidget *child = glade_xml_build_widget(xml, childnode, longname);
...
static void clist_build_children(GladeXML *xml, GtkWidget *w, GNode *node,
			       const char *longname) {
  GNode *childnode;
  gint col = 0;
  for (childnode = node->children; childnode; childnode = childnode->next) {
...
static void notebook_build_children(GladeXML *xml, GtkWidget *w, GNode *node,
			       const char *longname) {
  /* the notebook tabs are listed after the pages, and have child_name set to
   * Notebook:tab.  We store pages in a GList while waiting for tabs */
  GList *pages = NULL;
  GNode *childnode;
  for (childnode = node->children; childnode; childnode = childnode->next) {
    GtkWidget *child = glade_xml_build_widget(xml, childnode, longname);
...
static void dialog_build_children(GladeXML *xml, GtkWidget *w, GNode *node,
			       const char *longname) {
  xmlNodePtr info;
  GNode *childnode;
  ...
  if (content) free(content);
  for (childnode = node->children->children;
...
static GtkWidget *label_new(GladeXML *xml, GNode *node) {
  GtkWidget *label;
...
static GtkWidget *accellabel_new(GladeXML *xml, GNode *node) {
  GtkWidget *label;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
  char *string = NULL;
  GtkJustification just = GTK_JUSTIFY_CENTER;
...
static GtkWidget *entry_new(GladeXML *xml, GNode *node) {
  GtkWidget *entry;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
  char *text = NULL;
...
static GtkWidget *text_new(GladeXML *xml, GNode *node) {
  GtkWidget *wid;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
  char *text = NULL;
...
static GtkWidget *togglebutton_new(GladeXML *xml, GNode *node) {
  GtkWidget *button;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
  char *string = NULL;
...
static GtkWidget *checkbutton_new(GladeXML *xml, GNode *node) {
  GtkWidget *button;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
  char *string = NULL;
...
static GtkWidget *radiobutton_new(GladeXML *xml, GNode *node) {
  GtkWidget *button;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
  char *string = NULL;
  gboolean active = FALSE, draw_indicator = TRUE;
...
static GtkWidget *optionmenu_new(GladeXML *xml, GNode *node) {
...
static GtkWidget *combo_new(GladeXML *xml, GNode *node) {
...
static GtkWidget *list_new(GladeXML *xml, GNode *node) {
...
static GtkWidget *clist_new(GladeXML *xml, GNode *node) {
  GtkWidget *clist;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
  int cols = 1;
...
static GtkWidget *ctree_new(GladeXML *xml, GNode *node) {
  GtkWidget *ctree;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
  int cols = 1;
  GtkPolicyType hpol = GTK_POLICY_ALWAYS, vpol = GTK_POLICY_ALWAYS;
...
static GtkWidget *tree_new(GladeXML *xml, GNode *node) {
...
static GtkWidget *spinbutton_new(GladeXML *xml, GNode *node) {
...
static GtkWidget *hscale_new(GladeXML *xml, GNode *node) {
  GtkAdjustment *adj = get_adjustment((xmlNodePtr)node->data);
...
static GtkWidget *vscale_new(GladeXML *xml, GNode *node) {
  GtkAdjustment *adj = get_adjustment((xmlNodePtr)node->data);
...
static GtkWidget *hruler_new(GladeXML *xml, GNode *node){
...
static GtkWidget *vruler_new(GladeXML *xml, GNode *node){
...
static GtkWidget *hscrollbar_new(GladeXML *xml, GNode *node) {
  GtkAdjustment *adj = get_adjustment((xmlNodePtr)node->data);
...
static GtkWidget *vscrollbar_new(GladeXML *xml, GNode *node) {
  GtkAdjustment *adj = get_adjustment((xmlNodePtr)node->data);
...
static GtkWidget *statusbar_new(GladeXML *xml, GNode *node) {
  return gtk_statusbar_new();
}

static GtkWidget *toolbar_new(GladeXML *xml, GNode *node) {
  GtkWidget *tool;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
...
static GtkWidget *progressbar_new(GladeXML *xml, GNode *node) {
  return gtk_progress_bar_new();
}

static GtkWidget *arrow_new(GladeXML *xml, GNode *node) {
  GtkWidget *arrow;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
...
static GtkWidget *pixmap_new(GladeXML *xml, GNode *node) {
  GtkWidget *pix;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
...
static GtkWidget *drawingarea_new(GladeXML *xml, GNode *node) {
  return gtk_drawing_area_new();
}

static GtkWidget *hseparator_new(GladeXML *xml, GNode *node) {
  return gtk_hseparator_new();
}

static GtkWidget *vseparator_new(GladeXML *xml, GNode *node) {
  return gtk_vseparator_new();
}

static GtkWidget *menubar_new(GladeXML *xml, GNode *node) {
  return gtk_menu_bar_new();
}

static GtkWidget *menu_new(GladeXML *xml, GNode *node) {
  return gtk_menu_new();
}

static GtkWidget *menuitem_new(GladeXML *xml, GNode *node) {
  GtkWidget *menuitem;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
  char *label = NULL;
  ...
  if (right) gtk_menu_item_right_justify(GTK_MENU_ITEM(menuitem));
  return menuitem;
}

static GtkWidget *checkmenuitem_new(GladeXML *xml, GNode *node) {
  GtkWidget *menuitem;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
  char *label = NULL;
...
static GtkWidget *radiomenuitem_new(GladeXML *xml, GNode *node) {
  GtkWidget *menuitem;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
  char *label = NULL;
  gboolean right = FALSE, active = FALSE, toggle = FALSE;
...
static GtkWidget *hbox_new(GladeXML *xml, GNode *node) {
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
  int spacing = 0;
...
static GtkWidget *table_new(GladeXML *xml, GNode *node) {
  GtkWidget *table;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
...
static GtkWidget *fixed_new(GladeXML *xml, GNode *node) {
  return gtk_fixed_new();
}

static GtkWidget *hbuttonbox_new(GladeXML *xml, GNode *node) {
...
static GtkWidget *vbuttonbox_new(GladeXML *xml, GNode *node) {
...
static GtkWidget *frame_new(GladeXML *xml, GNode *node) {
  GtkWidget *frame;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
  char *label = NULL;
...
static GtkWidget *aspectframe_new(GladeXML *xml, GNode *node) {
  GtkWidget *frame;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
  char *label = NULL;
...
static GtkWidget *hpaned_new(GladeXML *xml, GNode *node) {
...
static GtkWidget *vpaned_new(GladeXML *xml, GNode *node) {
...
static GtkWidget *handlebox_new(GladeXML *xml, GNode *node) {
  return gtk_handle_box_new();
}

static GtkWidget *notebook_new(GladeXML *xml, GNode *node) {
...
static GtkWidget *alignment_new(GladeXML *xml, GNode *node) {
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
...
static GtkWidget *eventbox_new(GladeXML *xml, GNode *node) {
  return gtk_event_box_new();
}

static GtkWidget *scrolledwindow_new(GladeXML *xml, GNode *node) {
...
static GtkWidget *viewport_new(GladeXML *xml, GNode *node) {
...
static GtkWidget *curve_new(GladeXML *xml, GNode *node) {
...
static GtkWidget *gammacurve_new(GladeXML *xml, GNode *node) {
...
static GtkWidget *colorselection_new(GladeXML *xml, GNode *node) {
...
static GtkWidget *preview_new(GladeXML *xml, GNode *node) {
  GtkWidget *preview;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
...
static GtkWidget *window_new(GladeXML *xml, GNode *node) {
  GtkWidget *win;
  xmlNodePtr info = ((xmlNodePtr)node->data)->childs;
  gint xpos = -1, ypos = -1;
...
static GtkWidget *dialog_new(GladeXML *xml, GNode *node) {
...
static GtkWidget *colorselectiondialog_new(GladeXML *xml, GNode *node) {
  /* XXXX - fix this */
